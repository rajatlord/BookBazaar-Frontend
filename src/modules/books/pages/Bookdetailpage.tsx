...
import {
  Button, Tabs, Rate, Tag, Spin, message, Breadcrumb, InputNumber, Divider,
} from 'antd'
import {
  ShoppingCartOutlined, ArrowLeftOutlined,
  BookOutlined, ShopOutlined,
...
const BookDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (reviewsRes.status === 'fulfilled') setReviews(reviewsRes.value.data.data.items)
      } catch {
  ...
  const handleAddToCart = async () => {
    if (!isAuthenticated) { message.warning('Please sign in first'); return }
    if (!book) return
    setAddingCart(true)
    try {
          ...
          <div style={styles.metaGrid}>
            {[
              ...
              <div key={label}>
                ...
                <Typography variant="bodyMedium" style={{ marginTop: 2 }}>{value}</Typography>
              </div>
            ))}
          </div>
          {book.description && (
            <>
              <Divider />
              <Typography variant="bodyMedium" color="secondary" style={{ lineHeight: 1.8 }}>
                {book.description}
              </Typography>
            </>
          )}
            ...
            reviews.map((r) => (
              <div key={r.id} style={styles.reviewItem}>
                <div style={styles.reviewHeader}>
                  ...
                  <div>
                    <Typography variant="label">{r.buyer?.name ?? 'Reader'}</Typography>
                    <Rate disabled value={r.rating} style={{ fontSize: 12, display: 'block' }} />
                  </div>
                  {r.isVerifiedPurchase && (
                    <Tag color="green" style={{ marginLeft: 'auto', borderRadius: 980 }}>Verified</Tag>
                  )}
                </div>
                {r.comment && (
                  <Typography variant="bodySmall" color="secondary" style={{ marginTop: 8 }}>
                    {r.comment}
                  </Typography>
                )}
              </div>
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <Breadcrumb
          ...
          items={[
            { title: <span onClick={() => navigate('/books')} style={styles.breadLink}>Books</span> },
            { title: book.title },
          ]}
        />
        ...
        <div style={styles.layout}>
          ...
          <div style={styles.leftCol}>
            <div style={{ ...styles.bookCover, background: getGenreColor(book.genre) }}>
              <Typography variant="h3Regular" style={{ color: '#fff', textAlign: 'center', letterSpacing: '-0.02em' }}>
                {book.title}
              </Typography>
              <Typography variant="bodySmall" style={{ color: 'rgba(255,255,255,0.75)', marginTop: 12 }}>
                {book.author}
              </Typography>
              {book.genre && (
                <Tag style={{ marginTop: 20, borderRadius: 980, border: 'none', background: 'rgba(255,255,255,0.2)', color: '#fff' }}>
                  {book.genre}
                </Tag>
              )}
            </div>
            ...
            {book.shop && (
              <div style={styles.shopCard}>
                <ShopOutlined style={{ color: colors.primary, marginRight: 8 }} />
                <div>
          ...
          <div style={styles.rightCol}>
            {/* Status badge */}
            <Tag color={getBookStatusColor(book.status)} style={{ borderRadius: 980, marginBottom: 12 }}>
            ...
            <Typography variant="bodyLarge" color="secondary" style={{ marginBottom: 16 }}>
              by {book.author}
            </Typography>
            ...
            {/* Price */}
            <Typography variant="h1Regular" style={{ color: colors.textPrimary, letterSpacing: '-0.03em', marginBottom: 4 }}>
              ₹{book.price.toLocaleString('en-IN')}
            </Typography>
            <Typography variant="caption" color="secondary" style={{ marginBottom: 24, display: 'block' }}>
              {isOutOfStock
            ...
            {!isOutOfStock && (
              <div style={styles.purchaseRow}>
                <div style={styles.qtyWrap}>
                  <Typography variant="caption" color="secondary" style={{ marginBottom: 4, display: 'block' }}>Qty</Typography>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  detailsTab: {},
  metaGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px 32px' },
  reviewItem: {
    padding: '16px 0',
