...
const AdminShopsPage: React.FC = () => {
  ...
  const handleVerify = async () => {
    ...
    setSubmitting(true)
    try {
      await adminApi.verifyShop(selectedShop.id, action, reason || undefined)
      ...
      setSelectedShop(null); setAction(null); setReason('')
    } catch (err) {
  ...
  const SHOP_COLORS = ['#0071e3', '#5856d6', '#ff9500', '#34c759', '#af52de', '#ff3b30', '#30b0c7']
  const getShopColor = (id: string) => SHOP_COLORS[id.charCodeAt(0) % SHOP_COLORS.length]
    ...
    <div style={styles.page}>
      <div style={styles.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {total > 0 && (
            <Badge count={total} style={{ background: '#ff9500' }} overflowCount={99} />
          )}
        </div>
      </div>
      ...
      <div style={styles.container}>
        {loading ? (
          ...
          <div style={styles.center}>
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: '#e8faf0', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
              <CheckOutlined style={{ fontSize: 32, color: '#34c759' }} />
            </div>
            <Typography variant="h3Regular" style={{ marginBottom: 8 }}>All caught up!</Typography>
            <Typography variant="bodyMedium" color="secondary">No shops pending review.</Typography>
          </div>
        ) : (
          <Row gutter={[16, 16]}>
            {shops.map((shop) => (
              <Col key={shop.id} xs={24} sm={12} lg={8}>
                <Card style={styles.shopCard}>
                  {/* Shop color header */}
                  <div style={{ ...styles.shopStrip, background: getShopColor(shop.id) }}>
                    <div style={styles.shopAvatar}>
                      <Typography variant="h3Regular" style={{ color: getShopColor(shop.id), lineHeight: 1 }}>
                  ...
                  <div style={{ padding: '16px 16px 12px' }}>
                    <Typography variant="h4Semibold" style={{ marginBottom: 8 }}>
                      {shop.name}
                    </Typography>
                    {shop.description && (
                      <Typography variant="bodySmall" color="secondary" style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      <CalendarOutlined style={{ color: colors.textTertiary, fontSize: 11 }} />
                      <Typography variant="caption" color="secondary">
                        Applied {new Date(shop.createdAt).toLocaleDateString('en-IN')}
                      </Typography>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <Button
                        danger
                        icon={<CloseOutlined />}
                        onClick={() => { setSelectedShop(shop); setAction('REJECT'); setReason('') }}
                      ...
                      <Button
                        type="primary"
                        icon={<CheckOutlined />}
                        onClick={() => { setSelectedShop(shop); setAction('APPROVE'); setReason('') }}
                        style={{ flex: 1, borderRadius: 980, background: '#34c759', borderColor: '#34c759' }}
                      >
                        Approve
                      </Button>
                    </div>
      ...
      <Modal
        ...
        onCancel={() => { setSelectedShop(null); setAction(null) }}
        onOk={handleVerify}
        okText={action === 'APPROVE' ? 'Approve shop' : 'Reject shop'}
        okButtonProps={{ danger: action === 'REJECT', loading: submitting, style: { borderRadius: 980 }, disabled: action === 'REJECT' && !reason.trim() }}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  shopCard:  { borderRadius: 16, border: 'none', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', overflow: 'hidden', padding: 0 },
  shopStrip: { height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  shopAvatar:{ width: 52, height: 52, borderRadius: 16, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' },
}
